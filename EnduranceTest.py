...
class EnduranceTest(BenchTester.BenchTest):
  ...
  def run_test(self, testname, testvars={}):
    ...
    mozmillinst = mozmill.MozMill.create(runner_args=dict(binary=self.tester.binary),
                                         profile_args=dict(preferences={'startup.homepage_welcome_url' : '',
    ...
    try:
      ...
      results = mozmillinst.finish()
      successful = len(results.fails) == 0
    except Exception, e:
      try:
