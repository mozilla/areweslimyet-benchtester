...
class CompileBuild(Build):
  def __init__(self, repo, mozconfig, objdir, pull=False, commit=None, log=None):
    ...
    if self._commit:
      ...
      mercurial.commands.log(hg_ui, repo, rev=["limit(sort(limit(sort((:%(p)s - ::%(p)s):: and %(p)s::, \"rev\"), 1) or $p, \"-rev\"), 1)" % { 'p' : self._commit }], template="{node} {date}", date="", user=None, follow=None)
      commitinfo = hg_ui.popbuffer().split()
